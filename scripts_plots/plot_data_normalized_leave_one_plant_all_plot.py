...
MODIFICHE:
- Colori distinti per Training vs Test, senza linee tratteggiate
- Numero di samples individuali = numero di samples scelti dall'utente
...
def plot_2d_comparison(
    ...
    Args:
        ...
        n_samples_per_plant: Numero di samples da plottare
    """
    ...
    for cls in sorted(np.unique(train_data["y"])):
        ...
        if vis_mode in [2, 3]:
            # Training samples - USA IL NUMERO DI SAMPLES SCELTO
            n_train_to_plot = min(n_samples_per_plant, len(train_class_data))
            for i in range(n_train_to_plot):
                fig.add_trace(
                    go.Scatter(
                        x=frequencies,
            ...
            # Excluded samples - USA IL NUMERO DI SAMPLES SCELTO
            n_excl_to_plot = min(n_samples_per_plant, len(excl_class_data))
            for i in range(n_excl_to_plot):
                fig.add_trace(
                    go.Scatter(
                        x=frequencies,
                        ...
                        line=dict(color=colors_test[cls], width=1),
                        opacity=0.5,
        ...
        if vis_mode in [1, 3]:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    line=dict(color=colors_test[cls], width=3),
            ...
            # Plot Imaginary Part - Excluded Mean
            fig.add_trace(
                go.Scatter(
                    x=frequencies,
                    y=excl_imag_mean,
    ...
    fig.update_layout(
        ...
        + f"Training: {train_plants} vs Test: {leave_plant} | Samples mostrati: {n_samples_per_plant}",
        height=900,
        showlegend=True,
        ...
        if vis_mode in [1, 3]:
            ...
            fig.add_trace(
                go.Scatter3d(
                    ...
                    mode="lines+markers",
                    name=f"{class_names[cls]} Test {leave_plant} (media)",
                    line=dict(color=colors_test[cls], width=5),
...
def main():
    ...
    if vis_mode in [2, 3]:
        print(f"   📊 Samples individuali mostrati: {n_samples_per_plant}")
    print("=" * 70)
    print("\n   ℹ️  I grafici mostrano la differenza tra:")
    print(
        f"      - Piante di training {train_plants} (tonalità calde: verde/arancione/rosso)"
    )
    print(f"      - Pianta esclusa {leave_plant} (tonalità fredde: blu/viola/magenta)")
