"""
plot_frequency_trend_per_plant_plotly_3D.py

Script interattivo per visualizzare i trend di frequenza (media ± std) dell'impedenza
separatamente per ogni pianta, con colori distinti.
...
- Ogni pianta ha colori e linee distinte
...
def compute_mean_std_per_class_plant(X, y, plant_ids, freq_indices):
    """
    Calcola media e std dei dati per ogni classe E pianta separatamente.
    ...
    Returns:
        stats_dict (dict): Dizionario gerarchico con statistiche
            {plant: {class_id: {'real_mean': ..., 'real_std': ..., ...}}}
    """
    ...
    for plant in np.unique(plant_ids):
        ...
        X_plant = X[plant_mask]
        y_plant = y[plant_mask]

        stats_dict[plant] = {}

        # Per ogni classe all'interno della pianta
        for cls in np.unique(y_plant):
            cls_mask = y_plant == cls
            X_cls = X_plant[cls_mask]
...
def get_plant_class_colors():
    """
    Definisce colori per combinazioni pianta-classe.

    Ogni pianta ha una propria famiglia di colori.
    Ogni classe ha una propria sfumatura (chiara per Control, media per Early, scura per Late).

    Returns:
        color_dict (dict): Dizionario di colori
            {plant: {class: color_hex}}
    """
    # Colori per pianta
    plant_colors = {
        "P0": {  # TONALITÀ 1: BLU, ARANCIONE, VERDE
            0: "#1E90FF",  # Control - Blu reale (DodgerBlue)
            1: "#FF8C00",  # Early - Arancione scuro (DarkOrange)
            2: "#32CD32",  # Late - Verde lime (LimeGreen)
        },
        "P1": {  # TONALITÀ 2: ROSSO, VIOLA, TEAL
            0: "#DC143C",  # Control - Rosso cremisi (Crimson)
            1: "#9932CC",  # Early - Viola scuro (DarkOrchid)
            2: "#00CED1",  # Late - Teal/Cyan (DarkTurquoise)
        },
        "P3": {  # TONALITÀ 3: GIALLO, MAGENTA, MARRONE
            0: "#FFD700",  # Control - Oro/Giallo (Gold)
            1: "#FF1493",  # Early - Rosa acceso (DeepPink)
            2: "#8B4513",  # Late - Marrone saddle (SaddleBrown)
        },
    }
...
def create_frequency_trend_2d(stats_dict, stress_type, freq_indices, normalized=False):
    ...
    Ogni pianta ha colori diversi.

    Args:
        stats_dict (dict): Statistiche gerarchiche per pianta e classe
        stress_type (str): 'water' o 'iron'
        freq_indices (np.ndarray): Indici delle frequenze
    ...
    """
    # Mappatura classi → nomi e Plant → colori
    class_names = {0: "Control", 1: "Early_Stress", 2: "Late_Stress"}
    plant_class_colors = get_plant_class_colors()
    ...
    fig = make_subplots(
        ...
        cols=1,
        subplot_titles=(
            "Full Spectrum - Real Part per Pianta e Classe",
            "Full Spectrum - Imaginary Part per Pianta e Classe",
        ),
        specs=[[{"secondary_y": False}], [{"secondary_y": False}]],
    )

    # Aggiungi tracce per ogni pianta e classe
    for plant in sorted(stats_dict.keys()):
        plant_stats = stats_dict[plant]

        for cls in sorted(plant_stats.keys()):
            stats = plant_stats[cls]
            freq = stats["freq"]
            ...
            # Ottieni colore per questa combinazione pianta-classe
            color = plant_class_colors[plant][cls]
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=real_mean,
                    mode="lines",
                    name=f"{plant} - {class_names[cls]}",
                    line=dict(color=color, width=2.5),
                    legendgroup=f"{plant}_{cls}",
                    showlegend=True,
                ),
                row=1,
                col=1,
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=np.concatenate(
                        [real_mean + real_std, (real_mean - real_std)[::-1]]
                    ),
                    fill="toself",
                    fillcolor=color,
                    opacity=0.15,
            ...
            fig.add_trace(
                go.Scatter(
                    x=np.concatenate([freq, freq[::-1]]),
                    y=np.concatenate(
                        [imag_mean + imag_std, (imag_mean - imag_std)[::-1]]
                    ),
                    fill="toself",
                    fillcolor=color,
                    opacity=0.15,
    ...
    fig.update_layout(
        title=dict(
            text=f"Trend di Frequenza per Pianta (Full Spectrum){norm_suffix}<br>"
...
def create_frequency_trend_3d(stats_dict, stress_type, freq_indices, normalized=False):
    ...
    fig.update_layout(
        title=dict(
            text=f"3D Trend di Frequenza per Pianta: Real vs Imaginary vs Frequency{norm_suffix}<br>"
            f"<sub>Stress Type: {stress_type.upper()}</sub>",
            x=0.5,
            xanchor="center",
            font=dict(size=20),
...
def save_plot(
    ...
    filename = f"frequency_trend_perplant_{stress_type}_{plants_str}_classes{classes_str}_{freq_str}{norm_str}{type_str}.html"
...
def get_user_input():
    ...
    """
    print("=" * 80)
    print("VISUALIZZAZIONE TREND DI FREQUENZA DELL'IMPEDENZA (PER PIANTA)")
    print("=" * 80)
    ...
    print("  NOTA: Ogni pianta avrà colori distinti")
    plants_input = input(
    ...
    print("  0 = Control (chiaro)")
    print("  1 = Early Stress (medio)")
    print("  2 = Late Stress (scuro)")
...
def main():
    ...
    # Calcola statistiche per classe E pianta
    print("\n[INFO] Calcolo media e std per pianta e classe...")
    stats_dict = compute_mean_std_per_class_plant(
        X_filtered, y_filtered, plant_ids_filtered, freq_indices
    )
    ...
    for plant in sorted(stats_dict.keys()):
        plant_stats = stats_dict[plant]
        for cls in sorted(plant_stats.keys()):
            n_samples = plant_stats[cls]["n_samples"]
            class_names = {0: "Control", 1: "Early", 2: "Late"}
            print(f"      {plant} - {class_names[cls]}: {n_samples} campioni")
