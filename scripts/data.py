"""
Script analisi dataset piante 
Genera 4 gruppi di grafici per analizzare stress nelle piante
"""
...
# Aumenta il DPI e le dimensioni per permettere zoom migliore
DPI_GROUPED = 200  # Era 120 
DPI_OPTIMAL = 200  # Era 150 
DPI_FULL_SPECTRUM = 200  # Era 150 
DPI_INDIVIDUAL = 150  # Era 100 

FIGSIZE_GROUPED = (20, 18)  # Era (16, 14) 
FIGSIZE_OPTIMAL_COMP = (22, 10)  # Mantiene buon rapporto
FIGSIZE_OPTIMAL_TRENDS = (18, 14)  # Era (18, 12) 
FIGSIZE_FULL_SPEC_COMP = (22, 10)  # Mantiene buon rapporto
FIGSIZE_FULL_SPEC_TRENDS = (20, 14)  # Era (20, 12) 
FIGSIZE_INDIVIDUAL = (12, 10)  # Era (12, 8) 
...
def load_dataset(stress_type):
    ...
    # 200 frequenze, 2 componenti (Real, Imaginary)
...
def print_statistics(X, y, plant_ids):
    ...
    for cls in np.unique(y):
        count = np.sum(y == cls)
        print(f"  {CLASS_NAMES[cls]:15} = {count:4} campioni ({count/len(y)*100:5.1f}%)")
...
def plot_grouped_by_plant_class(X, y, plant_ids):
    ...
    MODIFICHE: Risoluzione aumentata a 200 DPI, figsize aumentate
    """
    print("\n=== GRUPPO 1: GROUPED_BY_PLANT_CLASS ===")
    print("Generazione grafici per pianta e classe (8 subplot per file)...")
    print(f"Risoluzione: {DPI_GROUPED} DPI | Dimensioni: {FIGSIZE_GROUPED[0]}x{FIGSIZE_GROUPED[1]}")
        ...
        for cls in np.unique(y):
            ...
            for subplot_idx in range(8):
                ...
                for idx in group_indices:
                    ...
                    ax.semilogx(frequencies, real, color=CLASS_COLORS[cls], 
                               alpha=0.15, linewidth=0.8, linestyle='-')
                    # Imaginary in linea tratteggiata
                    ax.semilogx(frequencies, imag, color='purple', 
                               alpha=0.15, linewidth=0.8, linestyle='--')

                # Media del gruppo (linea SPESSA)
                real_mean = X[group_indices, :, 0].mean(axis=0)
                imag_mean = X[group_indices, :, 1].mean(axis=0)
                ax.semilogx(frequencies, real_mean, 'b-', linewidth=2.5, 
                           label='Media Real', alpha=0.9)
                ax.semilogx(frequencies, imag_mean, 'm--', linewidth=2.5, 
                           label='Media Imag', alpha=0.9)

                # Evidenzia frequenze ottimali (linee verdi punteggiate)
                for freq_idx in OPTIMAL_FREQ_IDX:
                    ax.axvline(frequencies[freq_idx], color='green', 
                              linestyle=':', alpha=0.5, linewidth=1.5)

                ax.set_xlabel('Frequenza (Hz)', fontsize=10)
                ax.set_ylabel('Impedenza', fontsize=10)
                ax.set_title(f'Gruppo {subplot_idx+1} ({len(group_indices)} campioni)', 
                            fontsize=11, fontweight='bold')
                ax.legend(loc='best', fontsize=8)
                ax.grid(True, alpha=0.3)

            # Titolo generale
            fig.suptitle(f'Pianta {plant} | {class_name} | Totale: {len(indices)} campioni', 
...
def plot_optimal_frequencies(X, y):
    ...
    MODIFICHE: Risoluzione e dimensioni aumentate
    """
    print("\n=== GRUPPO 2: OPTIMAL_FREQUENCIES ===")
    ...
    for cls in np.unique(y):
        ...
        ax1.scatter(real, imag, c=CLASS_COLORS[cls], label=CLASS_NAMES[cls], 
    ...
    bp1 = ax2.boxplot(data_real, positions=positions-0.4, widths=0.6, patch_artist=True,
                      boxprops=dict(facecolor='lightblue', linewidth=2),
                      medianprops=dict(color='darkblue', linewidth=3))
    bp2 = ax2.boxplot(data_imag, positions=positions+0.4, widths=0.6, patch_artist=True,
    ...
    ax2.legend([bp1["boxes"][0], bp2["boxes"][0]], ['Real Part', 'Imaginary Part'], 
              fontsize=12)
    ax2.grid(True, alpha=0.3, axis='y')
    ax2.tick_params(labelsize=11)

    plt.suptitle(f'Analisi Frequenze Ottimali - {STRESS_TYPE.upper()} Stress', 
    ...
    for cls in np.unique(y):
        ...
        ax1.plot(frequencies_opt, real_mean, color=CLASS_COLORS[cls], 
                linewidth=3, marker='o', markersize=12, label=CLASS_NAMES[cls], alpha=0.8)
        ax1.fill_between(frequencies_opt, real_mean-real_std, real_mean+real_std,
                         color=CLASS_COLORS[cls], alpha=0.15)

        # Imaginary Part
        imag_mean = X_opt[mask, :, 1].mean(axis=0)
        imag_std = X_opt[mask, :, 1].std(axis=0)
        ax2.plot(frequencies_opt, imag_mean, color=CLASS_COLORS[cls], 
                linewidth=3, marker='s', markersize=12, label=CLASS_NAMES[cls], alpha=0.8)
        ax2.fill_between(frequencies_opt, imag_mean-imag_std, imag_mean+imag_std,
                         color=CLASS_COLORS[cls], alpha=0.15)

    ax1.set_xscale('log')
    ax1.set_xlabel('Frequenza (Hz)', fontsize=12)
    ax1.set_ylabel('Real Part (Media ± Std)', fontsize=12)
    ax1.set_title(f'Trend Real Part attraverso le 3 frequenze ottimali', fontsize=13)
    ...
    ax2.set_title(f'Trend Imaginary Part attraverso le 3 frequenze ottimali', fontsize=13)
    ax2.legend(fontsize=11)
    ax2.grid(True, alpha=0.3)
    ax2.tick_params(labelsize=11)

    plt.suptitle(f'Trend Frequenze Ottimali ({FREQ_RANGE}) - {STRESS_TYPE.upper()} Stress', 
                fontsize=16, fontweight='bold')
    plt.tight_layout()
...
def plot_full_spectrum(X, y):
    ...
    plt.suptitle(f'Full Spectrum Analysis - {STRESS_TYPE.upper()} Stress', 
                fontsize=16, fontweight='bold')
    plt.tight_layout()
    ...
    for cls in np.unique(y):
        ...
        ax1.semilogx(frequencies, real_mean, color=color, linewidth=2.5, 
                    label=label, alpha=0.8)
        ax1.fill_between(frequencies, real_mean-real_std, real_mean+real_std,
                         color=color, alpha=0.1)

        # Imaginary Part
        imag_mean = X[mask, :, 1].mean(axis=0)
        imag_std = X[mask, :, 1].std(axis=0)
        ax2.semilogx(frequencies, imag_mean, color=color, linewidth=2.5, 
                    label=label, alpha=0.8)
        ax2.fill_between(frequencies, imag_mean-imag_std, imag_mean+imag_std,
                         color=color, alpha=0.1)

    # Evidenzia frequenze ottimali
    for freq_idx in OPTIMAL_FREQ_IDX:
        ax1.axvline(frequencies[freq_idx], color='green', linestyle=':', 
                   alpha=0.5, linewidth=2)
        ax2.axvline(frequencies[freq_idx], color='green', linestyle=':', 
                   alpha=0.5, linewidth=2)
    ...
    plt.suptitle(f'Trend di Frequenza (Full Spectrum) - {STRESS_TYPE.upper()} Stress', 
                fontsize=16, fontweight='bold')
    plt.tight_layout()
...
def plot_individual_samples(X, y, plant_ids):
    ...
    print(f"Risoluzione: {DPI_INDIVIDUAL} DPI | Dimensioni: {FIGSIZE_INDIVIDUAL[0]}x{FIGSIZE_INDIVIDUAL[1]}")
    ...
    for idx in range(len(X)):
        ...
        ax1.semilogx(frequencies, real, 'b-', label='Real Part', 
                    linewidth=1.5, alpha=0.8)
        ax1.semilogx(frequencies, imag, 'r-', label='Imaginary Part', 
                    linewidth=1.5, alpha=0.8)

        # Evidenzia frequenze ottimali
        for freq_idx in OPTIMAL_FREQ_IDX:
            ax1.axvline(frequencies[freq_idx], color='green', 
                       linestyle='--', alpha=0.5, linewidth=1)

        ax1.set_xlabel('Frequenza (Hz)', fontsize=11)
        ax1.set_ylabel('Impedenza', fontsize=11)
        ax1.set_title(f'Campione #{idx} | Pianta {plant_id} | {class_name}', 
        ...
        # Evidenzia frequenze ottimali
        for freq_idx in OPTIMAL_FREQ_IDX:
            ax2.axvline(frequencies[freq_idx], color='green', 
                       linestyle='--', alpha=0.5, linewidth=1)

        ax2.set_xlabel('Frequenza (Hz)', fontsize=11)
        ax2.set_ylabel('Magnitudine |Z|', fontsize=11)
        ax2.set_title(f'Magnitudine Impedenza | Freq. ottimali: {FREQ_RANGE}', 
                     fontsize=11)
        ax2.grid(True, alpha=0.3)
...
def main():
    ...
    print(" VERSIONE AD ALTA RISOLUZIONE (200 DPI)")
    print("="*60)

    # Input configurazione
    print("\nCONFIGURAZIONE:")
    stress_input = input("Quale dataset analizzare? (water/iron) [default: water]: ").strip().lower()
    ...
    print(" (Risoluzione aumentata per zoom migliore)")
    print("="*60)

    # GRUPPO 1: Grafici per pianta/classe
    plot_grouped_by_plant_class(X, y, plant_ids)

    # GRUPPO 2: Frequenze ottimali
    plot_optimal_frequencies(X, y)

    # GRUPPO 3: Full spectrum
    plot_full_spectrum(X, y)
    ...
    print("="*60)
    print("\nGRAFICI GENERATI (AD ALTA RISOLUZIONE):")
    print(f"  📁 ./plots/grouped_by_plant_class/  (9 file @ {DPI_GROUPED} DPI)")
    print(f"  📁 ./plots/optimal_frequencies/     (2 file @ {DPI_OPTIMAL} DPI)")
    print(f"  📁 ./plots/full_spectrum/           (2 file @ {DPI_FULL_SPECTRUM} DPI)")
    if GENERATE_INDIVIDUAL:
        print(f"  📁 ./plots/individual_samples/      (2016 file @ {DPI_INDIVIDUAL} DPI)")

    print("\n📊 MIGLIORAMENTI DI RISOLUZIONE:")
    print(f"   - Grouped by Plant: +67% (120 DPI → {DPI_GROUPED} DPI)")
    print(f"   - Optimal Frequencies: +33% (150 DPI → {DPI_OPTIMAL} DPI)")
    print(f"   - Full Spectrum: +33% (150 DPI → {DPI_FULL_SPECTRUM} DPI)")
    print(f"   - Individual Samples: +50% (100 DPI → {DPI_INDIVIDUAL} DPI)")

    print("\n🔍 ZOOM: Ora puoi zummare su tutti i grafici per leggere meglio!")
    print(f"Dataset analizzato: {STRESS_TYPE.upper()} STRESS")
