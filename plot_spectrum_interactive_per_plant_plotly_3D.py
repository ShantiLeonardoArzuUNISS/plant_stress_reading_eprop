"""
plot_spectrum_interactive_per_plant_plotly_3D.py

Script interattivo per visualizzare lo spettro di impedenza Real vs Imaginary
separatamente per ogni pianta, con colori distinti per pianta e classe.

Features:
- Visualizzazione interattiva con Plotly (2D e 3D)
- Ogni pianta ha colori distinti
...
def get_plant_class_colors():
    ...
    """
    # Colori per pianta (tinte principali)
    plant_colors = {
...
def create_interactive_plot_2d(
    ...
    """
    Crea plot 2D interattivo (Real vs Imaginary) con piante separate.
    ...
    # Itera su piante e classi per creare tracce separate
    unique_plants = np.unique(plant_ids)
    unique_classes = np.unique(y)

    for plant in sorted(unique_plants):
        for cls in unique_classes:
            # Filtra per pianta e classe
    ...
    fig.update_layout(
        title=dict(
            text=f"2D Spectrum: Real vs Imaginary per Pianta ({freq_min:.0f} Hz - {freq_max:.2e} Hz){norm_suffix}<br>"
...
def create_interactive_plot_3d(
    ...
    """
    Crea plot 3D interattivo (Real vs Imaginary vs Frequency) con piante separate.
    ...
    fig.update_layout(
        title=dict(
            text=f"3D Spectrum: Real vs Imaginary vs Frequency per Pianta ({freq_min:.0f} Hz - {freq_max:.2e} Hz){norm_suffix}<br>"
            f"<sub>Stress Type: {stress_type.upper()}</sub>",
            x=0.5,
            xanchor="center",
            font=dict(size=20),
...
def save_plot(
    ...
    filename = f"spectrum_perplant_{stress_type}_{plants_str}_classes{classes_str}_{freq_str}{norm_str}{type_str}.html"
...
def get_user_input():
    ...
    """
    print("=" * 80)
    print("VISUALIZZAZIONE INTERATTIVA SPETTRO IMPEDENZA (PER PIANTA)")
    print("=" * 80)
